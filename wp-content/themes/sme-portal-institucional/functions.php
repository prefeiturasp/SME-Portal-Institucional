	...
	if( ($_GET['filter'] == 'grupo' && $user->roles[0] == 'contributor' && $_GET['post_type'] == 'page') 
		...
		if($variable && $variable != ''){
            foreach($variable as $grupo){
				$selectPages = get_post_meta($grupo, 'selecionar_paginas', true);
				$selectContatos = get_post_meta($grupo, 'contatos_sme', true);
				if($selectPages != ''){
					$pages[] = $selectPages;
				}
				if($selectContatos != ''){
					$pages[] = $selectContatos;
				}
			}
        }
...
function fpw_post_info( $id, $post ) {
    ...
    if( has_post_thumbnail( $post->ID ) ){
		...
		$placeholder_id = get_field( 'imagem_placeholder', 'placeholders' )['ID'] ?? 28528;
...
add_action( "compromissocl_edit_form", 'hide_description_row');
add_action( "compromissocl_add_form", 'hide_description_row');
add_action( "compromissogn_edit_form", 'hide_description_row');
add_action( "compromissogn_add_form", 'hide_description_row');
add_action( "compromissoip_edit_form", 'hide_description_row');
add_action( "compromissoip_add_form", 'hide_description_row');
add_action( "compromissoit_edit_form", 'hide_description_row');
add_action( "compromissoit_add_form", 'hide_description_row');
add_action( "compromissojt_edit_form", 'hide_description_row');
add_action( "compromissojt_add_form", 'hide_description_row');
add_action( "compromissope_edit_form", 'hide_description_row');
add_action( "compromissope_add_form", 'hide_description_row');
add_action( "compromissopi_edit_form", 'hide_description_row');
add_action( "compromissopi_add_form", 'hide_description_row');
add_action( "compromissosa_edit_form", 'hide_description_row');
add_action( "compromissosa_add_form", 'hide_description_row');
add_action( "compromissosma_edit_form", 'hide_description_row');
add_action( "compromissosma_add_form", 'hide_description_row');
add_action( "compromissosmi_edit_form", 'hide_description_row');
add_action( "compromissosmi_add_form", 'hide_description_row');
...
function remove_date_categ_drop(){
	...
	if ( 'concurso' == $screen->post_type || 'contato' == $screen->post_type || 'agendanew' == $screen->post_type || str_contains($screen->post_type, 'agenda-dre')){		
		?>
			<style>
				.postform{
					display: none;
	...
	?>
		...
		<form action="<?= get_template_directory_uri(); ?>/export-users.php">
			<select name="funcao" id="">
				...
				<option value="publicador">Colaborador Publicador</option>
...
add_filter('acf/fields/relationship/query/key=field_6376989e42256', 'filter_by_user_group', 10, 3);
...
function editar_concursos(){
	...
	if(!current_user_can('administrator')){
		...
		if(isset($result) && is_array($result) && !in_array('1', $result)){
...
// Exibe o setor na pagina de perfil do usuario
...
function user_profile_fields( $user ) {
	if(!current_user_can('administrator')){
		// recebe o valor do custom field
		$setor = get_user_meta( $user->ID, 'setor', true );
		

	?>
			<h2 style="color: #000000;">Setor</h2>
			<table class="form-table">
				<tr>
					<th><label for="setor">Setor</label></th>
					<td>
						<input type="text" name="setor" id="setor" value="<?php echo esc_attr( $setor ) ?>" class="regular-text" disabled />
					</td>
				</tr>				
			</table>
	<?php
...
function remove_sub_sub_menu($views){
    ...
    $remove_views = [ 'all','publish','future','sticky','draft','pending','trash', 'private', 'mine', 'yoast_cornerstone' ];
...
function limit_contact_posts($query) {
	...
	if( $_GET['post_type'] == 'educacao-faq' && $user->roles[0] != 'administrator' && $_GET['filter'] == 'grupo' ) {
		...
		if($variable && $variable != ''){
            foreach($variable as $grupo){
				$categ_faqs[] = get_post_meta($grupo, 'faqs_sme', true);
			}
        }
...
add_filter('views_edit-agenda-dre-cl', 'wp38990_add_movies_filter');
add_filter('views_edit-agenda-dre-gn', 'wp38990_add_movies_filter');
add_filter('views_edit-agenda-dre-ip', 'wp38990_add_movies_filter');
add_filter('views_edit-agenda-dre-it', 'wp38990_add_movies_filter');
add_filter('views_edit-agenda-dre-jt', 'wp38990_add_movies_filter');
add_filter('views_edit-agenda-dre-pe', 'wp38990_add_movies_filter');
add_filter('views_edit-agenda-dre-pi', 'wp38990_add_movies_filter');
add_filter('views_edit-agenda-dre-sa', 'wp38990_add_movies_filter');
add_filter('views_edit-agenda-dre-sma', 'wp38990_add_movies_filter');
add_filter('views_edit-agenda-dre-smi', 'wp38990_add_movies_filter');
...
add_filter('acf/fields/taxonomy/query/key=field_6515dcbe29798', 'my_acf_fields_taxonomy_query', 10, 3);
...
add_filter('acf/load_field/key=field_6515dcbe29798', 'acf_load_color_field_choices');
...
function render_custom_html_in_options_page($field) {
    // Verifica se estamos na página de opções correta
    if (isset($_GET['page']) && $_GET['page'] === 'acf-options-periodo-eleitoral') {
		
        // Verifica se o campo atual é o 'periodo_eleitoral'
        if ($field['_name'] === 'ocultar_paginas') {
            echo '<div class="custom-html-block">';
            echo '<h2>Página do período eleitoral</h2>';
            $pages = get_field('ocultar_paginas', 'periodo-eleitoral');
            if($pages){
				echo "<ul>";
				foreach($pages as $page){
					echo '<li>' . get_the_title($page) . ' - <a href="' . get_the_permalink($page) . '">' . get_the_permalink($page) . '</a></li>'; 
				}
				echo "</ul>";
			}
            echo '</div>';
...
add_action('acf/render_field', 'render_custom_html_in_options_page');
...
function registrar_taxonomia_editorias() {
    $labels = array(
        ...
        'singular_name'     => 'Core da Editoria',
...
function validar_editoria_radio_obrigatoria() {
    global $pagenow;
    if (!is_admin() || !in_array($pagenow, ['post.php', 'post-new.php'])) return;
    ?>
    <script>
    jQuery(document).ready(function($) {
