...
<head>
	...
	// Verificação para sorteios externos
    global $wp_query;

    if (isset($wp_query->sorteio_data) && !empty($wp_query->sorteio_data['title'])) {
        // Caso especial para sorteios externos
        ?>
        <title><?php echo esc_html($wp_query->sorteio_data['title']); ?> - <?php echo STM_SITE_NAME; ?></title>
        <meta name="description" content="<?php echo esc_attr(wp_strip_all_tags($wp_query->sorteio_data['title'])); ?> - <?php echo STM_SITE_DESCRIPTION; ?>"/>
        <?php
    } else {
        // Mantém sua lógica original para outros casos
        if (is_front_page()) {
            ...
            if (trim($descriptionCategoria != "")) { ?>
                <meta name="description" content="<?php echo $descriptionCategoria; ?>."/>
            <?php } else { ?>
                <meta name="description" content="<?php echo STM_SITE_NAME; ?>. <?php echo STM_SITE_DESCRIPTION; ?>"/>
            <?php }
        } else {
            ...
            if (trim($descriptionPagina != "")) { ?>
                <meta name="description" content="<?php echo $descriptionPagina; ?>."/>
            <?php } else { ?>
                <meta name="description" content="<?php wp_title('', true, '-'); ?> - <?php echo STM_SITE_DESCRIPTION; ?>"/>
            <?php }
        }
    }
    ...
    <script>
        ...
        const elementsResults = document.querySelectorAll('.no-results');
        if (elementsResults.length > 1) {
            Array.from(elementsResults).slice(1).forEach(el => el.remove());
        }

        document.addEventListener('DOMContentLoaded', function() {
            // Seleciona todas as divs com a classe "no-results"
            const noResultsDivs = document.querySelectorAll('.no-results');
                        
            // Se houver mais de uma div com essa classe
            if (noResultsDivs.length > 1) {
                // Itera sobre todas começando da segunda (índice 1)
                for (let i = 1; i < noResultsDivs.length; i++) {
                noResultsDivs[i].style.display = 'none';
                }
            }
        });
    </script>
...
<section class="row cabecalho-acessibilidade">
...
<section id="main" role="main">
    <header class="pref-menu fonte-dez mb-4">
            <section class="row cabecalho-cinza-claro">

                <section class="container">
                    <section class="row">
                        <article class="col-12 col-lg-6 col-xs-12 d-flex justify-content-start">
                            <ul class="list-inline menu-outros my-3 d-sm-flex">
                                <li class="list-inline-item mr-2 mr-lg-3"><a class="text-white" href="http://esic.prefeitura.sp.gov.br/Account/Login.aspx">Acesso à informação e-sic <span class="esconde-item-acessibilidade">(Link para um novo sítio)</span> </a></li>
                                <li class="list-inline-item mr-2 mr-lg-3"><a class="text-white" href="https://www.prefeitura.sp.gov.br/cidade/secretarias/ouvidoria/fale_com_a_ouvidoria/index.php?p=227268">Ouvidoria <span class="esconde-item-acessibilidade">(Link para um novo sítio)</span></a></li>
                                <li class="list-inline-item mr-2 mr-lg-3"><a class="text-white" href="http://transparencia.prefeitura.sp.gov.br/">Portal da Transparência <span class="esconde-item-acessibilidade">(Link para um novo sítio)</span></a></li>
                                <li class="list-inline-item mr-2 mr-lg-3"><a class="text-white" href="https://sp156.prefeitura.sp.gov.br/portal/servicos">SP 156 <span class="esconde-item-acessibilidade">(Link para um novo sítio)</span></a></li>
                                <li class="p-1 border rounded list-inline-item btn-acessibilidade-mobile d-inline-block d-md-none">
                                    <i class="fa fa-keyboard-o fa-lg" aria-hidden="true"></i><span class="esconde-item-acessibilidade">(Atalhos Acessíveis)</span>
                                </li>
                            </ul>
                        </article>
                        <article class="col-4 col-lg-6 col-xs-12 d-none d-lg-flex justify-content-end align-items-center">
                            <?php 
            ...
            <section class='row logo-principal'>
                ...
                <div class="container">
                    <div class="row pt-3 pb-0">
                        ...
                        <nav class="col-12 mt-3 navbar navbar-expand-lg menu-topo d-block d-lg-none">
                            <section class="container">
    ...
    <nav class="navbar navbar-expand-lg menu-topo d-none d-lg-block">
        <section class="container">
    ...
    <?php
        $certi = get_field('ativar_certi','certificacoes');
        $home = get_field('exibir_home','certificacoes');
        $link = get_field('url_redirect','certificacoes');
        $img = get_field('img_certi','certificacoes');
    ?>
    <?php
        if($certi && $img): 
            if (!$home || ($home && (is_front_page() || is_home()))): ?>
