...
class ArchiveAgendaAjaxCalendario extends Util
{
	...
	public function montaQueryAgenda($data_recebida_ao_clicar){
		...
		if ($query->have_posts()) : while ($query->have_posts()) : $query->the_post();
			?>
			<article class="col-lg-12 col-xs-12 ooi">
				...
				//converte campo hora por extenso para ordenar
				 ...
				 echo $hri=date('His',$hri);?></div>
					<div class="horario d-inline"><?= str_replace('h00', 'h', $this->getCamposPersonalizados('hora_do_evento')); ?> - <?= str_replace('h00', 'h', $this->getCamposPersonalizados('fim_do_evento')); ?></div> |
