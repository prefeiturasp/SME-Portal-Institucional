...
class ArchiveAgendaAjaxCalendarioDre extends Util
{
	...
	public function montaQueryAgenda($data_recebida_ao_clicar, $dre_select){
			...
			<article class="col-lg-12 col-xs-12">
				<div class="agenda mb-4 agenda-new agenda-dre">
					...
					<?php foreach($eventos as $evento): ?>
						<div class="agenda mb-4 agenda-inside aaa agenda-dre <?= $dre_select; ?>">
							<div class="order_hri">
								<?php
							...
							<div class="horario d-inline"><?= str_replace('h00', 'h', $evento['hora_evento']); ?> - <?= str_replace('h00', 'h', $evento['fim_evento']); ?></div> |
							<?php //if( $evento['nome_compromisso'] != '') :?>
								<div class="evento d-inline aaa"><?= $evento['nome_compromisso'] ?></div>
							<?php //else: ?>
								<div class="evento d-inline bbb"><?= get_term( $evento['compromisso'] )->name; ?></div>
							<?php //endif; ?>
							...
							<?php if($evento['participantes_evento'] != ''): ?>
								<div class="local part"><strong>Participantes: </strong> <?= $evento['participantes_evento']; ?></div>
							<?php endif; ?>
						</div>
					<?php endforeach; ?>
		...
		endwhile;
			...
			echo '<p class="agenda agenda-new agenda-dre aaa"><strong>Não existem eventos cadastrados nesta data</strong></p> - ' . $dre_select;
		endif;
		wp_reset_postdata();
