...
class Breadcrumb
{
	...
	{
		$title = get_field('nome_da_secretaria','conf-rodape');
		if($title && $title != ''){
	...
	public function the_breadcrumb(){
		...
		if ( !is_front_page() ) {
			...
			echo '<div class="row">';
			echo '<nav>';
			echo '<ol id="' . $this->breadcrums_id . '" class="' . $this->breadcrums_class . ' mb-0">';
			...
			if ( is_archive() && !is_tax() && !is_category() && !is_tag() ) {
				...
				if($post_type != 'post') {
					...
					if($wp_query->query['post_type'] == 'virtual_sorteio'){
						echo '<li class="item-cat item-custom-post-type-' . $post_type . '"><a class="bread-cat bread-custom-post-type-' . $post_type . '" href="' . get_home_url() . '/sorteios-e-beneficios-aos-estagiarios/">Sorteios e Beneficios aos Estagiários</a></li>';
						echo '<li class="separator"> ' . $this->separator . ' </li>';
					} else {
						echo '<li class="item-cat item-custom-post-type-' . $post_type . '"><a class="bread-cat bread-custom-post-type-' . $post_type . '" href="' . $post_type_archive . '">' . $post_type_object->labels->name . '</a></li>';
						echo '<li class="separator"> ' . $this->separator . ' </li>';
				...
				if(!empty($category)) {
					...
					foreach($category as $cat){
						$link = get_field('link', 'category_' . $cat->term_id);
				...
				if($wp_query->query['post_type'] == 'virtual_sorteio'){
					...
					echo '<li class="item-current item-' . $post->ID . '"><strong class="bread-current bread-' . $post->ID . '" >' . $sorteio_data['title'] . '</strong></li>';
				} else {
			...
			if($cat_show){
				echo '<ol class="breadcrumb bg-transparent pl-1 pt-0">';
					echo '<li><strong>Mais notícias DRE:</strong>&nbsp;</li>';
					$i = 0;
					foreach($cat_show as $cat){
						$nome = str_replace('Diretoria Regional de Educação', '', $cat['nome']);
						if($i == 0){
							echo '<li><a href="' . $cat['link'] . '">' . $nome . '</a></li>';
						} else {
							echo '<li>&nbsp;/ <a href="' . $cat['link'] . '">' . $nome . '</a></li>';
						}
						$i++;
