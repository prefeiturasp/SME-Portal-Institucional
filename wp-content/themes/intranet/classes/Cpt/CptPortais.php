...
class CptPortais extends Cpt
{
	...
	function filter_categ_portais($query) {
		...
		if($_GET['categorias_portais']){
			$tax = array(
				array(
					'taxonomy' => 'categorias-portais',
					'field' => 'term_id',
					'terms' => $_GET['categorias_portais'],
				)
			);
	...
	public function exibe_cols($cols, $post_type){
		if ($post_type == $this->cptSlug) {
			...
			$cols['categorias-portais'] = 'Categoria';
			$cols['date'] = 'Data';
		}
		return $cols;
	}
	...
	public function cols_content($col){
		...
		switch ($col) {
			...
			case 'categorias-portais':
				$categorias = get_the_terms( $post->ID, 'categorias-portais' );
				//$terms_string = join(', ', wp_list_pluck($categorias, 'name'));
				$i = 0;
				foreach($categorias as $categoria){
					if($i == 0){
						echo '<a href="' . get_home_url() . '/wp-admin/edit.php?post_type=portais&categorias_portais=' . $categoria->term_id . '">' . $categoria->name . '</a>';
					} else {
						echo ', <a href="' . get_home_url() . '/wp-admin/edit.php?post_type=portais&categorias_portais=' . $categoria->term_id . '">' . $categoria->name . '</a>';
					}
					$i++;
	...
	public function register(){
		...
		register_taxonomy(
			...
			array(
				"hierarchical" => true,
				"label" => 'Categorias de Portais',
				"singular_label" => 'Categoria do Portal',	
			)
		);
