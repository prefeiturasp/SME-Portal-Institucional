...
class WP_Post_Meta {
	...
	public function get_post_meta_single( $post, $meta_key, array $meta_key_attrs ) {
		...
		if ( array_key_exists( $meta_key, $post_meta ) ) {
			...
			if($meta_key == 'localizacao'){
				if (function_exists('get_post_field')){
					$slug = get_post_field( 'post_name', $post_meta_value[0] );
					if($slug){
						$post_meta_value = $slug;
					} else {
						$post_meta_value = 'nenhuma-localizacao';
					}
				} else {
					$post_meta_value = 'nenhuma-unidade';
				}
				
			}
			
		} elseif( $meta_key == 'localizacao' ){
			$post_meta_value = 'multiplas-unidades';
		} else {
