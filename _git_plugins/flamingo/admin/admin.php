...
require_once FLAMINGO_PLUGIN_DIR . '/admin/includes/SimpleXLSXGen.php';
...
function flamingo_load_inbound_admin() {
	...
	if ( ! empty( $_GET['export'] ) ) {
		...
		$filename = ( empty( $sitename ) ? '' : $sitename . '-' )
			. sprintf( 'flamingo-inbound-%s.xlsx', wp_date( 'Y-m-d' ) );
		...
		if( $_GET['channel_id'] == 17) {
			...
			foreach($items as $item){
				$contatos[] = array(
					...
					$item->fields['rf'],
					$item->fields['cargo'],
					$item->fields['cargo-outros'],
					$item->fields['dre'],
					$item->fields['referer-page'],
					$item->meta['date'] . ' - ' . $item->meta['time'],
				);
			}
		} elseif( $_GET['channel_id'] == 30) {
			$contatos[] = array(
				'Estrelas',
				'Mais Gostou',
				'Tres Outras',
			...
			foreach($items as $item){
				$contatos[] = array(
					$item->fields['starrating-414'],
					$item->fields['mais-gostou'],
					$item->fields['tres-outras'],
					$item->fields['gostaria-melhorar'],
					$item->fields['adicionar-mais'],
					$item->fields['referer-page'],
					$item->meta['date'] . ' - ' . $item->meta['time'],
				);
			}
		} elseif( $_GET['channel_id'] == 18) {
			$contatos[] = array(
				'RF',
				'E-mail',				
				'Pag Referencia',
		...
		$xlsx = Classes\Lib\SimpleXLSXGen::fromArray( $contatos );
