...
COPY phpconf/php.ini "/etc/php/7.4/cli/php.ini"
COPY phpconf/fpm-status.conf "/etc/php/7.4/fpm/pool.d/fpm-status.conf"
...
RUN find /var/www/html -type d -exec chmod 775 {} \; && find /var/www/html -type f -exec chmod 644 {} \; && chmod 660 /var/www/html/wp-config.php && chmod 664 /var/www/html/.htaccess
RUN chmod +x /usr/local/bin/docker-php-entrypoint && dos2unix /usr/local/bin/docker-php-entrypoint
